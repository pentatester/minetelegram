...
const { windowItemsToString, itemByName } = require('../utils')
const { noInstance } = require('../commons')

function prettyStringWindow (window) {
  return `Title  : ${window.title}
  Items :\n${windowItemsToString(window)}
  `
}

let afterOpened = name => {
  return `
Chest ${name} opened, now you can :
/chest - Get curenly opened chest
...
function openChest (ctx) {
  ...
  if (bot) {
    ...
    if (bot.currentWindow) {
      ...
      if (args.length === 2) {
        let command = args[1]
        let chest = openChestOrCommand(command, Chest)
        ...
        try {
          const chest = openChestOrCommand(chestBlock)
          chest.on('open', chestOpen)
          chest.on('close', chestClose)
          chest.on('updateSlot', updateSlot)
          bot.currentChest = chest
  ...
  // Events
  function chestClose () {
    ctx.reply('Chest closed.')
  }
  function chestOpen (title = '') {
    ctx.reply(afterOpened(title))
  }
  function updateSlot (oldItem, newItem) {}

  // Chest command implementation
  function openChestOrCommand (command, Class) {
